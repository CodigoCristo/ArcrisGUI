#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <assert.h>
#include <stdarg.h>
#include <glib.h>
#include "page4.h"

// Mock de funciones de logging para las pruebas
#define LOG_INFO(format, ...) printf("[INFO] " format "\n", ##__VA_ARGS__)
#define LOG_ERROR(format, ...) printf("[ERROR] " format "\n", ##__VA_ARGS__)
#define LOG_WARNING(format, ...) printf("[WARNING] " format "\n", ##__VA_ARGS__)

// Declaraciones de funciones que necesitamos para las pruebas
gboolean page4_load_reserved_usernames(Page4Data *data);
gboolean page4_is_username_valid(const gchar *username, Page4Data *data);
gboolean page4_is_hostname_valid(const gchar *hostname, Page4Data *data);
gboolean page4_is_password_length_valid(const gchar *password);

// Función para crear datos de prueba
Page4Data* create_test_page4_data() {
    Page4Data *data = g_malloc0(sizeof(Page4Data));
    
    // Cargar nombres reservados
    page4_load_reserved_usernames(data);
    
    return data;
}

// Función para limpiar datos de prueba
void cleanup_test_page4_data(Page4Data *data) {
    if (data) {
        if (data->reserved_usernames) {
            g_strfreev(data->reserved_usernames);
        }
        g_free(data);
    }
}

// Pruebas de validación de usuario
void test_username_validation() {
    printf("\n=== PRUEBAS DE VALIDACIÓN DE USUARIO ===\n");
    
    Page4Data *data = create_test_page4_data();
    
    // Casos válidos
    assert(page4_is_username_valid("usuario", data) == TRUE);
    assert(page4_is_username_valid("user123", data) == TRUE);
    assert(page4_is_username_valid("mi_usuario", data) == TRUE);
    assert(page4_is_username_valid("a", data) == TRUE);
    printf("✓ Casos válidos de usuario pasaron\n");
    
    // Casos inválidos - empieza con mayúscula
    assert(page4_is_username_valid("Usuario", data) == FALSE);
    assert(page4_is_username_valid("ADMIN", data) == FALSE);
    printf("✓ Casos inválidos con mayúscula inicial pasaron\n");
    
    // Casos inválidos - empieza con número
    assert(page4_is_username_valid("1usuario", data) == FALSE);
    assert(page4_is_username_valid("9admin", data) == FALSE);
    printf("✓ Casos inválidos con número inicial pasaron\n");
    
    // Casos inválidos - nombres reservados
    assert(page4_is_username_valid("root", data) == FALSE);
    assert(page4_is_username_valid("admin", data) == FALSE);
    assert(page4_is_username_valid("daemon", data) == FALSE);
    assert(page4_is_username_valid("sys", data) == FALSE);
    printf("✓ Casos inválidos con nombres reservados pasaron\n");
    
    // Casos inválidos - cadena vacía
    assert(page4_is_username_valid("", data) == FALSE);
    assert(page4_is_username_valid(NULL, data) == FALSE);
    printf("✓ Casos inválidos con cadena vacía/NULL pasaron\n");
    
    cleanup_test_page4_data(data);
    printf("✓ Todas las pruebas de validación de usuario pasaron\n");
}

// Pruebas de validación de hostname
void test_hostname_validation() {
    printf("\n=== PRUEBAS DE VALIDACIÓN DE HOSTNAME ===\n");
    
    Page4Data *data = create_test_page4_data();
    
    // Casos válidos
    assert(page4_is_hostname_valid("miequipo", data) == TRUE);
    assert(page4_is_hostname_valid("laptop123", data) == TRUE);
    assert(page4_is_hostname_valid("pc_trabajo", data) == TRUE);
    assert(page4_is_hostname_valid("servidor", data) == TRUE);
    printf("✓ Casos válidos de hostname pasaron\n");
    
    // Casos inválidos - empieza con mayúscula
    assert(page4_is_hostname_valid("MiEquipo", data) == FALSE);
    assert(page4_is_hostname_valid("SERVIDOR", data) == FALSE);
    printf("✓ Casos inválidos con mayúscula inicial pasaron\n");
    
    // Casos inválidos - empieza con número
    assert(page4_is_hostname_valid("1servidor", data) == FALSE);
    assert(page4_is_hostname_valid("9laptop", data) == FALSE);
    printf("✓ Casos inválidos con número inicial pasaron\n");
    
    // Casos inválidos - nombres reservados
    assert(page4_is_hostname_valid("root", data) == FALSE);
    assert(page4_is_hostname_valid("admin", data) == FALSE);
    assert(page4_is_hostname_valid("daemon", data) == FALSE);
    printf("✓ Casos inválidos con nombres reservados pasaron\n");
    
    // Casos inválidos - cadena vacía
    assert(page4_is_hostname_valid("", data) == FALSE);
    assert(page4_is_hostname_valid(NULL, data) == FALSE);
    printf("✓ Casos inválidos con cadena vacía/NULL pasaron\n");
    
    cleanup_test_page4_data(data);
    printf("✓ Todas las pruebas de validación de hostname pasaron\n");
}

// Pruebas de validación de longitud de contraseña
void test_password_length_validation() {
    printf("\n=== PRUEBAS DE VALIDACIÓN DE LONGITUD DE CONTRASEÑA ===\n");
    
    // Casos válidos (3 o más caracteres)
    assert(page4_is_password_length_valid("123") == TRUE);
    assert(page4_is_password_length_valid("abc") == TRUE);
    assert(page4_is_password_length_valid("password") == TRUE);
    assert(page4_is_password_length_valid("micontraseña123") == TRUE);
    printf("✓ Casos válidos de longitud de contraseña pasaron\n");
    
    // Casos inválidos (menos de 3 caracteres)
    assert(page4_is_password_length_valid("") == FALSE);
    assert(page4_is_password_length_valid("1") == FALSE);
    assert(page4_is_password_length_valid("ab") == FALSE);
    assert(page4_is_password_length_valid(NULL) == FALSE);
    printf("✓ Casos inválidos de longitud de contraseña pasaron\n");
    
    printf("✓ Todas las pruebas de validación de longitud de contraseña pasaron\n");
}

// Prueba de carga de nombres reservados
void test_reserved_usernames_loading() {
    printf("\n=== PRUEBAS DE CARGA DE NOMBRES RESERVADOS ===\n");
    
    Page4Data *data = create_test_page4_data();
    
    // Verificar que se cargaron nombres reservados
    assert(data->reserved_usernames != NULL);
    printf("✓ Lista de nombres reservados cargada\n");
    
    // Verificar que contiene nombres conocidos
    gboolean found_root = FALSE;
    gboolean found_admin = FALSE;
    gboolean found_daemon = FALSE;
    
    for (int i = 0; data->reserved_usernames[i]; i++) {
        if (g_strcmp0(data->reserved_usernames[i], "root") == 0) {
            found_root = TRUE;
        } else if (g_strcmp0(data->reserved_usernames[i], "admin") == 0) {
            found_admin = TRUE;
        } else if (g_strcmp0(data->reserved_usernames[i], "daemon") == 0) {
            found_daemon = TRUE;
        }
    }
    
    assert(found_root == TRUE);
    assert(found_admin == TRUE);
    assert(found_daemon == TRUE);
    printf("✓ Nombres reservados conocidos encontrados (root, admin, daemon)\n");
    
    cleanup_test_page4_data(data);
    printf("✓ Todas las pruebas de carga de nombres reservados pasaron\n");
}

// Pruebas de casos límite y especiales
void test_edge_cases() {
    printf("\n=== PRUEBAS DE CASOS LÍMITE ===\n");
    
    Page4Data *data = create_test_page4_data();
    
    // Nombres con caracteres especiales
    assert(page4_is_username_valid("user_name", data) == TRUE);
    assert(page4_is_username_valid("user-name", data) == TRUE);
    printf("✓ Nombres con guiones y guiones bajos son válidos\n");
    
    // Nombres muy largos
    char long_name[256];
    memset(long_name, 'a', 255);
    long_name[255] = '\0';
    assert(page4_is_username_valid(long_name, data) == TRUE);
    printf("✓ Nombres muy largos son válidos\n");
    
    // Contraseñas en el límite
    assert(page4_is_password_length_valid("123") == TRUE);  // Exactamente 3 caracteres
    assert(page4_is_password_length_valid("12") == FALSE);  // Menos de 3 caracteres
    printf("✓ Contraseñas en el límite de 3 caracteres funcionan correctamente\n");
    
    cleanup_test_page4_data(data);
    printf("✓ Todas las pruebas de casos límite pasaron\n");
}

// Función principal de pruebas
int main() {
    printf("=== INICIANDO PRUEBAS DE VALIDACIÓN DE PÁGINA 4 ===\n");
    
    // Ejecutar todas las pruebas
    test_username_validation();
    test_hostname_validation();
    test_password_length_validation();
    test_reserved_usernames_loading();
    test_edge_cases();
    
    printf("\n=== TODAS LAS PRUEBAS COMPLETADAS EXITOSAMENTE ===\n");
    printf("✓ Validaciones de usuario implementadas correctamente\n");
    printf("✓ Validaciones de hostname implementadas correctamente\n");
    printf("✓ Validaciones de contraseña implementadas correctamente\n");
    printf("✓ Carga de nombres reservados implementada correctamente\n");
    
    return 0;
}