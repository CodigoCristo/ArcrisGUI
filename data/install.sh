...
SNAPCONF
        ...
        if chroot /mnt /bin/bash -c "snapper list-configs" | grep -q "root"; then
            ...
            if chroot /mnt /bin/bash -c "snapper -c root create --description 'Sistema recién instalado - Estado inicial'"; then
                echo -e "${GREEN}✓ Snapshot inicial de root creado exitosamente${NC}"
            else
                echo -e "${YELLOW}Warning: No se pudo crear snapshot inicial de root${NC}"
            fi
        else
            echo -e "${YELLOW}Warning: Configuración root no encontrada, saltando snapshot inicial de root${NC}"
        fi

        # Verificar que la configuración de home existe antes de crear snapshot
        if chroot /mnt /bin/bash -c "snapper list-configs" | grep -q "home"; then
            echo -e "${CYAN}Creando snapshot inicial de /home...${NC}"
            if chroot /mnt /bin/bash -c "snapper -c home create --description 'Home recién instalado - Estado inicial'"; then
                echo -e "${GREEN}✓ Snapshot inicial de /home creado exitosamente${NC}"
            else
                echo -e "${YELLOW}Warning: No se pudo crear snapshot inicial de /home${NC}"
            fi
        else
            echo -e "${YELLOW}Warning: Configuración home no encontrada, saltando snapshot inicial de /home${NC}"
        fi
